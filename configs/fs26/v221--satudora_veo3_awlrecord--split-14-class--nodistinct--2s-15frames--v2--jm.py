graph = "coco_headless"
work_dir = f"/home/laptq/laptq-fs26-shoplifting-detection/runs/ACLNet/fs26/v221--satudora_veo3_awlrecord--split-14-class--nodistinct--2s-15frames--v2--jm"
...
model = dict(
    ...
    cls_head=dict(type="SimpleHead", data_cfg=graph, num_classes=14, in_channels=384),
)
...
train_pipeline = [
    ...
    dict(type="GenSkeFeat", dataset="coco_headless", feats=['jm']),
...
evaluation = dict(
    interval=1,
    metrics=["harmonic_mean_recall"],
    greater_keys=["harmonic_mean_recall"],
    class_map=None,
    class_weights=[1, 1, 1, 1, 1, 1, 1, 1.5, 1.5, 1.5, 1.5, 2.0, 2.0, 2.0],
)
